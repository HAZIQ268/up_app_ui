...
class _multanState extends State<multan> {
  ...
  Future<void> fetchData() async {
    try {
      final userdata = await FirebaseFirestore.instance.collection('multan').get();
  ...
  void deleteDialog(String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Delete Attraction', style: TextStyle(color: Colors.deepOrange.shade800)),
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () {
                        if (formKey.currentState!.validate()) {
                          ...
                          FirebaseFirestore.instance.collection('multan').add(newProduct).then((_) {
                            fetchData();
                            Navigator.of(context).pop();
                          });
                        }
  ...
  Widget _buildSortButton(String label, IconData icon) {
    return OutlinedButton.icon(
      icon: Icon(icon, size: 16, color: _sortCriteria == label ? Colors.deepOrange : Colors.grey),
      label: Text(label, style: TextStyle(color: _sortCriteria == label ? Colors.deepOrange : Colors.grey)),
      style: OutlinedButton.styleFrom(
        side: BorderSide(color: _sortCriteria == label ? Colors.deepOrange : Colors.grey.shade300),
